...
function buildLocallyControlledSeatSetForCurrentMode() {
...
function refreshLocallyControlledSeatSetForMode() {
    LOCAL_CONTROLLED_SEAT_SET = buildLocallyControlledSeatSetForCurrentMode();
}

function applyUserNaturalPositionFor4P(userNaturalPosition) {
    // Local 4P keeps control/display synchronized for now; semantics remain distinct.
    ...
    activeLocalControlSeat = localControlledPlayerIndex;
    refreshLocallyControlledSeatSetForMode();
}

// ---------------------------------------------------------------------------
// Test mode: local control includes the primary seat plus its afterhand.
// ---------------------------------------------------------------------------
let TEST_MODE = false;
let LOCAL_CONTROLLED_SEAT_SET = buildLocallyControlledSeatSetForCurrentMode();
function isLocallyControlledSeat(player) { return LOCAL_CONTROLLED_SEAT_SET.has(player); }
...
let activeLocalControlSeat = localControlledPlayerIndex;
...
function renderAllHands() {
    renderHand(activeLocalControlSeat);
}

function renderHand(player) {
    if (!isLocallyControlledSeat(player)) return;
    if (player !== activeLocalControlSeat) return;
...
function renderPauseHumanAgreementControls() {
    ...
    let seats = Array.from(LOCAL_CONTROLLED_SEAT_SET).filter(seat => seat !== pauseState.requesterSeat);
...
/**
 * Start shot clock + bank time for a locally controlled seat's move.
...
/**
 * Show "No declaration" button in center area for locally controlled seats.
...
/**
 * Auto-play as bot when locally controlled seat time runs out.
 */
...
function runDealingPhase() {
    ...
    dealingTimer = setInterval(function () {
        ...
        for (let hp of LOCAL_CONTROLLED_SEAT_SET) {
        ...
        if (game.phase === GamePhase.DEALING && game.dealingStage === DealingStage.FINAL_DECLARATION_CALL) {
            clearInterval(dealingTimer);
            dealingTimer = null;
...
function runFinalDeclarationWindow() {
    if (isPauseDialogBlockingGameplay()) return;
    if (game && game.phase === GamePhase.DEALING) {
        game.dealingStage = DealingStage.FINAL_DECLARATION_CALL;
    }
...
// ---------------------------------------------------------------------------
// Final declaration call resolver (last action window inside dealing phase)
// ---------------------------------------------------------------------------
...
function promptCurrentPlayer() {
    ...
    if (isLocallyControlledSeat(cp)) {
        // Switch displayed hand to the active locally controlled seat.
        activeLocalControlSeat = cp;
        clearSelection();
